...
(() => {
  ...
  const API_KEYS_KEY = "__codexLiveTokenCostApiKeysV1";
  const BALANCE_CACHE_KEY = "__codexLiveTokenCostBalanceCacheV1";
  ...
  const FALLBACK_DEFAULT_PRICES = {
    ...
    "deepseek-v4-flash": { input: 1, cachedInput: 0.02, output: 2 },
    "deepseek-v4-pro": { input: 3, cachedInput: 0.025, output: 6 },
  };
  ...
  function fmtMoney(value, model) {
    const n = Number(value);
    const symbol = (model && DEEPSEEK_MODEL_PATTERN.test(String(model))) ? "\u00a5" : "$";
  ...
  // --- API Keys & Balance ---
  function getApiKeys() {
    try { return JSON.parse(localStorage.getItem(API_KEYS_KEY) || "{}"); } catch (e) { return {}; }
  }
  ...
  function getBalanceCache() {
    try { return JSON.parse(localStorage.getItem(BALANCE_CACHE_KEY) || "null"); } catch (e) { return null; }
  }

  function saveBalanceCache(cache) {
    try { localStorage.setItem(BALANCE_CACHE_KEY, JSON.stringify(cache)); } catch (e) {}
  }

  function balanceFetchViaBridge(url, headers) {
    var bridge = window.electronBridge;
    if (!bridge || typeof bridge.sendMessageFromView !== "function") return Promise.reject(new Error("bridge unavailable"));
    var requestId = "cltc-balance-" + Date.now() + "-" + Math.random().toString(36).slice(2);
    return new Promise(function(resolve, reject) {
      ...
      var handler = function(event) {
        ...
        cleanup();
        if (data.responseType !== "success" || (data.status | 0) >= 400) {
          reject(new Error("failed: " + (data.status || data.responseType)));
          return;
        }
        try { resolve(JSON.parse(data.bodyJsonString || "{}")); } catch (e) { reject(e); }
      };
      window.addEventListener("message", handler, true);
      timer = window.setTimeout(function() { cleanup(); reject(new Error("timeout")); }, 10000);
      bridge.sendMessageFromView({ type: "fetch", method: "GET", url: url, requestId: requestId, headers: headers });
    });
  }

  var balanceProviders = {
    deepseek: {
      label: "DeepSeek",
      key: "deepseek",
      async fetch(apiKey) {
        var data = await balanceFetchViaBridge(DEEPSEEK_BALANCE_URL, { "Authorization": "Bearer " + apiKey });
        var info = (data && data.balance_infos && data.balance_infos[0]) || {};
        return { provider: "deepseek", currency: info.currency || "CNY", balance: info.total_balance || "0", raw: data };
      }
    }
  };

  function fetchAllBalances() {
    if (state.balanceFetching) return;
    var keys = getApiKeys();
    var hasKeys = Object.values(keys).some(function(v) { return v && v.length > 10; });
    if (!hasKeys) return;
    state.balanceFetching = true;
    var providers = Object.keys(balanceProviders);
    var remaining = providers.length;
    var newData = Object.assign({}, state.balanceData || {});
    providers.forEach(function(name) {
      var apiKey = keys[name];
      if (!apiKey) { remaining--; return; }
      balanceProviders[name].fetch(apiKey).then(function(result) {
        newData[result.provider] = result;
      }).catch(function() {
        newData[name] = newData[name] || { provider: name, error: true };
      }).finally(function() {
        remaining--;
        if (remaining <= 0) {
          state.balanceData = newData;
          saveBalanceCache(newData);
          state.balanceFetching = false;
          scheduleRender();
        }
      });
    });
    if (remaining <= 0) state.balanceFetching = false;
  }

  function scheduleBalanceRefresh(initial) {
    if (state.balanceFetchTimer) return;
    if (initial) {
      var cache = getBalanceCache();
      if (cache) state.balanceData = cache;
      fetchAllBalances();
    }
    state.balanceFetchTimer = window.setInterval(function() {
      fetchAllBalances();
    }, BALANCE_REFRESH_INTERVAL_MS);
  }

  function saveApiKeyFromEditor() {
    var root = settingsEditorRoot();
    var input = root && root.querySelector("[data-api-field='deepseek-key']");
    if (!input) return;
    var keys = getApiKeys();
    keys.deepseek = String(input.value || "").trim();
    saveApiKeys(keys);
    state.balanceData = null;
    state.balanceFetching = false;
    fetchAllBalances();
    if (!state.balanceFetchTimer) scheduleBalanceRefresh(false);
  }

  function hideTurnCost() {
    try { return localStorage.getItem(HIDE_TURN_COST_KEY) === "true"; } catch (e) { return false; }
  }

  function hideSessionCost() {
    try { return localStorage.getItem(HIDE_SESSION_COST_KEY) === "true"; } catch (e) { return false; }
  }

  function saveHideTurnCost(v) {
    try { localStorage.setItem(HIDE_TURN_COST_KEY, v ? "true" : "false"); } catch (e) {}
  }

  function saveHideSessionCost(v) {
    try { localStorage.setItem(HIDE_SESSION_COST_KEY, v ? "true" : "false"); } catch (e) {}
  }
  ...
  function parseModelEffortText(value) {
    ...
    const raw = text.match(/\b(gpt-[a-z0-9._-]+|o\d[a-z0-9._-]*|deepseek-[a-z0-9._-]+)\b/i)?.[1] || "";
  ...
  function priceFor(model) {
    ...
    const overridePrice = normalizePrice(overrides[key]);
    if (overridePrice && (overridePrice.input > 0 || overridePrice.output > 0)) return overridePrice;
    if (overridePrice) return DEFAULT_PRICES[key] || overridePrice;
  ...
  function calcCost(usage, price, options = {}) {
    ...
    var multiplier = options.fastMode === true ? fastModeCostMultiplier(options.model) : 1;
    var peakMult = deepseekPeakMultiplier(options.model);
    if (peakMult !== 1) multiplier *= peakMult;
    return {
  ...
  function liveSnapshot() {
    ...
    const turnCost = costForModelUsage(current, model, { fastMode: fastMode });
    return { current, session: displaySession, turns: turns.length, sessionKey, model, modelInfo, price, sessionCost, turnCost, dayCost, dayUsage, confidence, running, fastMode };
  }
  ...
  function ensureStyle() {
    ...
    style.textContent = `
      ...
      @keyframes cltc-balance-roll {
        0% { opacity: 0; transform: translateY(-8px); }
        100% { opacity: 1; transform: translateY(0); }
      }
      .cltc-balance-value.balance-updated {
        animation: cltc-balance-roll 0.4s ease-out;
      }
      .cltc-api-field {
        display: flex;
        flex-direction: column;
  ...
  function handleSettingsClick(event) {
    ...
    if (event.target.closest?.("[data-action='save-api-keys']")) {
      saveApiKeyFromEditor();
      renderSettingsOverlay(liveSnapshot());
      return;
    }
    const hideTurnCheckbox = event.target.closest?.("[data-misc-field='hideTurnCost']");
    if (hideTurnCheckbox) {
      saveHideTurnCost(Boolean(hideTurnCheckbox.checked));
      if (state.root) state.root.dataset.cltcSkeletonVersion = "";
      render(true);
      return;
    }
  ...
  function apiKeySettingsHtml() {
    ...
    return `
      <div class="cltc-settings-section">
        <div class="cltc-settings-title">API 密钥</div>
        <div class="cltc-muted">DeepSeek API Key 用于查询余额</div>
        <label class="cltc-api-field">
          <span>DeepSeek API Key</span>
          <input data-api-field="deepseek-key" type="password" value="${escapeHtml(keys.deepseek || "")}" placeholder="sk-...">
        </label>
        <div class="cltc-price-actions">
          <button type="button" data-action="save-api-keys">保存</button>
        </div>
      </div>
      <div class="cltc-settings-section">
        <div class="cltc-settings-title">显示选项</div>
        <label class="cltc-checkbox-field">
          <input type="checkbox" data-misc-field="hideTurnCost" ${hideTurnCost() ? "checked" : ""}>
          <span>隐藏本轮花费</span>
        </label>
        <label class="cltc-checkbox-field">
          <input type="checkbox" data-misc-field="hideSessionCost" ${hideSessionCost() ? "checked" : ""}>
          <span>隐藏会话花费</span>
        </label>
      </div>
    `;
  ...
  function hubSkeletonHtml() {
    ...
    const turnCostDisplay = hideTurnCost() ? "" : `<span class="cltc-pill">本轮花费 ${valueSlot("turn-cost")}</span>`;
    const sessionCostDisplay = hideSessionCost() ? "" : `<span class="cltc-pill">会话花费 ${valueSlot("session-cost")}${textSlot("session-priced-label")}</span>`;
    return `
      ...
      ${turnCostDisplay}
      ${sessionCostDisplay}
      <span class="cltc-pill"><span class="cltc-balance-label">余额 </span><span data-cltc-balance-key="deepseek" class="cltc-balance-value">-</span></span>
  ...
  function updateHubContent(root, snap, sessionPricedLabel, dayPricedLabel, effortLabel) {
    ...
    updateValueSlot(root, "turn-cost", fmtMoney(snap.turnCost.value, snap.model));
    updateValueSlot(root, "session-cost", fmtMoney(snap.sessionCost.value, snap.model));
    ...
    const balanceSlot = root.querySelector?.("[data-cltc-balance-key='deepseek']");
    if (balanceSlot && state.balanceData && state.balanceData.deepseek && !state.balanceData.deepseek.error) {
      var d = state.balanceData.deepseek;
      var newText = "\u00a5" + (Number(d.balance) || 0).toFixed(2);
      if (balanceSlot.textContent !== newText) {
        balanceSlot.textContent = newText;
        balanceSlot.classList.add("balance-updated");
        setTimeout(function() { balanceSlot.classList.remove("balance-updated"); }, 600);
      }
    }
  ...
  function start() {
    ...
    render();
    window.setTimeout(function() { scheduleBalanceRefresh(true); }, 2000);
  }
  ...
  function destroy() {
    ...
    if (state.balanceFetchTimer) { window.clearInterval(state.balanceFetchTimer); state.balanceFetchTimer = 0; }
